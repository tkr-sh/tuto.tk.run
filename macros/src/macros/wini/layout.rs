use {
    ...
    syn::{FnArg, Ident, PatType, parse_macro_input, spanned::Spanned},
};
    ...
    for (idx, input_it) in input.sig.inputs.iter_mut().enumerate() {
        ...
        match input_it {
            ...
            FnArg::Typed(pat_ty) => {
                            ...
                            (
                                {
                                    ...
                                    let error_msg = format!(
                                        "Invalid attribute macro: the argument's type doesn't implement `{impl_from_trait}`"
                                    );
                ...
                let not_a_valid_extractor_error = format!(
                    "{ty_str} is not a valid extractor since it does not implement either the `FromResponseBody`, `FromResponseParts` or `FromRequestParts` traits.",
                );
    ...
    for (idx, attr) in arg.attrs.iter().enumerate() {
        if let Ok(from) = FromTrait::from_str(
            ...
            if current_from.is_some() {
                return Err(
                    "You cannot have different `#[from_request_parts]`, `#[from_response_body]` or `#[from_response_parts]` for the same argument.",
                );
            } else {
